...
web_server:
	...
	@PID=$$(lsof -ti:$(WEB_SERVER_PORT)); if [ -n "$$PID" ]; then echo "Killing process $$PID using port $(WEB_SERVER_PORT)"; kill $$PID || echo "Could not kill process $$PID. Operation not permitted"; fi
...
ollama_server:
	...
	@PID=$$(pgrep -f "ollama serve"); if [ -n "$$PID" ]; then echo "Killing existing ollama server process $$PID"; kill $$PID || echo "Could not kill process $$PID. Operation not permitted"; fi
